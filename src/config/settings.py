"""Application configuration for CMDB Agent."""
...
class Settings(BaseSettings):
    ...
    app_name: str = Field(default="cmdb-agent", description="Application name")
    ...
    database_url: str = Field(
        default="postgresql+asyncpg://postgres:postgres@localhost:5432/cmdb_agent",
    ...
    opensearch_index: str = Field(default="cmdb_agent_vectors", description="Index")
    ...
    mcp_server_name: str = Field(default="cmdb-agent-mcp", description="MCP server name")

    a2a_discovery_url: str = Field(default="http://localhost:8500", description="A2A discovery URL")
    a2a_agent_id: str = Field(default="cmdb-agent", description="A2A agent ID")
